...
int main(int argc, char **argv)
{
    ...
    int fd, maxfd, clientfd,sd,reading;
    fd_set fdlist;
    struct client *p;
    char *s;
    socklen_t len;
    ...
    //check for proper number of arugments
    parseargs(argc, argv);


    if((fd = socket(AF_INET, SOCK_STREAM,0)) == 0){
        perror("socket");
        return(1); //do u exit if socket fails?
    }

    //copies the new line character to the first sizeof serv_addr characters 
    //of the string serv_addr
    memset(&serv_addr,'\0', sizeof serv_addr);
    //type of socket created
    serv_addr.sin_family = AF_INET;
    serv_addr.sin_addr.s_addr = INADDR_ANY;
    //use the static variable port to assign port number to the server
    serv_addr.sin_port = htons(port);

    //bind the socket to the localhost 
    if(bind(fd,(struct sockaddr *)&serv_addr, sizeof (serv_addr)) < 0){
        perror("bind");
        return(1);  //do u exit if bind fails?
    } 
    
    if(listen(fd,5)){
        perror("listen");
        return(1); //do u exit or return if listen fails?
    }
    ...
    while(1){
        ...
        //linked list
        //add sockets to the set until client is not null
        for(p = clientlist; p; p = p->next){
        //while(client->next){
            FD_SET(p->fd,&fdlist);           
            //compare current maxsd with the one just added
            //need highest fd for select()
            if(p->fd > maxfd)
                maxfd = p->fd;
        }

        //use select() to wait for an activity on one of the sockets        
        ...
        if(FD_ISSET(fd,&fdlist)){ //new clients
            len = sizeof(cli_addr);
            ...
            //send_string(clientfd,hello);  
            hellolen = strlen(hello);

            //user helper function add new connection
            newclient(clientfd,&cli_addr);
        }
        else{
            //iterate through all clients
            for(p = clientlist; p; p = p->next){
                sd = p->fd;
                if(FD_ISSET(sd,&fdlist)){
                    //if nothing to read, then remove the client
                    if((reading = read(sd, p->buf + p->bytes_in_buf,sizeof (p->buf - p->bytes_in_buf)) == 0)){
                        removeclient(p);
                    }
                    else{
                        s = memnewline(p->buf,strlen(p->buf));
                        if(s != NULL){
                            do_something(p, s);
                        }
                    }
...
static void do_say(struct client *p){
    ...
    //traverse through every client 
    //send message to client q, iff q is in the same room as client p
    for(q = clientlist; q; q = q->next){
        if(p->loc == q->loc){
            //send message to its self and other clients in the same room
            send_string(q->fd, p->buf);
            send_string(p->fd, p->buf);
        }
    }
}
